...
> **Rule:** The LLM generates the *human prose* for a `Metacognitive Reflection` strictly from the pre-validated fields of a `Reasoning Artifact` and `Evidence Chain`. It is prohibited from generating facts, nodes, edges, or scores.
